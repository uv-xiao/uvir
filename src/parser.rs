...
impl<'a> Parser<'a> {
        ...
        let name = match self.advance() {
            ...
            None => {
                return Err(Error::ParseError(
                    "Expected value reference, found EOF".to_string(),
                ))
            }
        };
    ...
    pub fn parse_type(&mut self) -> Result<TypeId> {
        ...
        match self.advance() {
            ...
            Some(Token::I1) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 1,
                signed: false,
            })),
            Some(Token::I8) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 8,
                signed: true,
            })),
            Some(Token::I16) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 16,
                signed: true,
            })),
            Some(Token::I32) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 32,
                signed: true,
            })),
            Some(Token::I64) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 64,
                signed: true,
            })),
            Some(Token::U8) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 8,
                signed: false,
            })),
            Some(Token::U16) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 16,
                signed: false,
            })),
            Some(Token::U32) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 32,
                signed: false,
            })),
            Some(Token::U64) => Ok(self.ctx.intern_type(TypeKind::Integer {
                width: 64,
                signed: false,
            })),

            // Built-in float types
            Some(Token::F16) => Ok(self.ctx.intern_type(TypeKind::Float {
                precision: FloatPrecision::Half,
            })),
            Some(Token::F32) => Ok(self.ctx.intern_type(TypeKind::Float {
                precision: FloatPrecision::Single,
            })),
            Some(Token::F64) => Ok(self.ctx.intern_type(TypeKind::Float {
                precision: FloatPrecision::Double,
            })),
            Some(Token::BF16) => Ok(self.ctx.intern_type(TypeKind::Float {
                precision: FloatPrecision::Half,
            })), // BF16 treated as Half for now
            ...
            Some(Token::Vector) => {
                ...
                let element_type = self.parse_type()?;
                self.expect_token(Token::RightAngle)?;
                Ok(self.ctx.intern_type(TypeKind::Vector {
                    shape,
                    element_type,
                }))
            }
            ...
            Some(Token::Tensor) => {
                ...
                let element_type = self.parse_type()?;
                self.expect_token(Token::RightAngle)?;
                Ok(self.ctx.intern_type(TypeKind::Tensor {
                    shape,
                    element_type,
                }))
            }
            ...
            Some(Token::GenericInteger(width)) => {
                let width = *width;
                Ok(self.ctx.intern_type(TypeKind::Integer {
                    width,
                    signed: true,
                }))
            }
            ...
            Some(Token::Bang) => {
                ...
                Err(Error::ParseError(
                    "Dialect types not yet implemented".to_string(),
                ))
            }
    ...
    pub fn parse_function_type(&mut self) -> Result<TypeId> {
        ...
        while !matches!(self.peek(), Some(Token::RightParen)) {
            ...
            if matches!(self.peek(), Some(Token::Comma)) {
                ...
                return Err(Error::ParseError(
                    "Expected ',' or ')' in function type".to_string(),
                ));
            }
        }
        ...
        if matches!(self.peek(), Some(Token::LeftParen)) {
            ...
            while !matches!(self.peek(), Some(Token::RightParen)) {
                ...
                if matches!(self.peek(), Some(Token::Comma)) {
                    ...
                    return Err(Error::ParseError(
                        "Expected ',' or ')' in function outputs".to_string(),
                    ));
                }
            }
            self.expect_token(Token::RightParen)?;
    ...
    pub fn parse_attribute(&mut self) -> Result<Attribute> {
        match self.peek() {
            ...
            Some(Token::LeftBracket) => {
                ...
                while !matches!(self.peek(), Some(Token::RightBracket)) {
                    ...
                    if matches!(self.peek(), Some(Token::Comma)) {
                        ...
                        return Err(Error::ParseError(
                            "Expected ',' or ']' in array attribute".to_string(),
                        ));
                    }
                }
            ...
            None => Err(Error::ParseError(
                "Expected attribute, found EOF".to_string(),
            )),
        }
    }
    ...
    pub fn parse_attribute_dict(&mut self) -> Result<AttributeMap> {
        ...
        while !matches!(self.peek(), Some(Token::RightBrace)) {
            ...
            if matches!(self.peek(), Some(Token::Comma)) {
                ...
                return Err(Error::ParseError(
                    "Expected ',' or '}' in attribute dict".to_string(),
                ));
            }
        }
    ...
    pub fn parse_operation(&mut self) -> Result<()> {
        ...
        let (dialect_name, op_name, is_generic) =
        ...
        let op_info = self
            ...
            .ok_or_else(|| {
                Error::ParseError(format!("Unknown operation {}.{}", dialect_name, op_name))
            })?;
            ...
            while !matches!(self.peek(), Some(Token::RightParen)) {
                if self.peek().map_or(false, |t| t.is_value_ref()) {
                    ...
                    if matches!(self.peek(), Some(Token::Comma)) {
                        ...
                        return Err(Error::ParseError(
                            "Expected ',' or ')' in operand list".to_string(),
                        ));
                    }
                } else {
                    break;
        ...
        if is_generic || matches!(self.peek(), Some(Token::Colon)) {
            if matches!(self.peek(), Some(Token::Colon)) {
                ...
                return Err(Error::ParseError(
                    "Generic operation syntax requires type signature".to_string(),
                ));
            }
            ...
            let output_types =
                if let Some(TypeKind::Function { outputs, .. }) = self.ctx.get_type(func_type) {
    ...
    pub fn parse_module(&mut self) -> Result<()> {
        ...
        if matches!(self.peek(), Some(Token::Module)) {
            ...
            while !matches!(self.peek(), Some(Token::RightBrace)) {
                if self.is_at_end() {
                    return Err(Error::ParseError(
                        "Unexpected EOF in module body".to_string(),
                    ));
                }
