...
fn verify_same_type(op: &OpData, ctx: &Context) -> Result<()> {
    ...
    for operand in &op.operands {
        if let Some(ty) = get_value_type(*operand, ctx) {
            ...
            return Err(Error::VerificationError(
                format!("Cannot find type for operand {:?}", operand)
            ));
        }
    }
    ...
    for result in &op.results {
        if let Some(ty) = get_value_type(*result, ctx) {
            ...
            return Err(Error::VerificationError(
                format!("Cannot find type for result {:?}", result)
            ));
        }
    }
    ...
    if !all_types.is_empty() {
        ...
        for ty in &all_types[1..] {
            if *ty != first_type {
                return Err(Error::VerificationError(
                    format!("SameTy trait violation: not all operands and results have the same type")
                ));
            }
        }
    }
...
mod tests {
    ...
    fn test_same_type_verification() {
        ...
        use crate::ops::{OpInfo, OpData, OpStorage};
