...
fn test_mixed_type_operations() {
    ...
    let region = ctx.get_global_region();
    let types: Vec<_> = region.values.values().map(|v| v.ty).collect();
...
fn test_operation_with_attributes() {
    ...
    let metadata = op.attributes.iter().find(|(k, _)| *k == metadata_key);
...
fn test_value_use_def_chains() {
    ...
    ctx.get_global_region_mut()
        ...
        .get_mut(vals[0])
        .unwrap()
        .defining_op = Some(uvir::ops::OpRef(const_ref));
    ...
    ctx.get_global_region_mut()
        ...
        .get_mut(vals[1])
        .unwrap()
        .defining_op = Some(uvir::ops::OpRef(add1_ref));
    ...
    ctx.get_global_region_mut()
        ...
        .get_mut(vals[2])
        .unwrap()
        .defining_op = Some(uvir::ops::OpRef(mul_ref));
    ...
    ctx.get_global_region_mut()
        ...
        .get_mut(vals[3])
        .unwrap()
        .defining_op = Some(uvir::ops::OpRef(add2_ref));
    ...
    for (i, &val) in vals.iter().enumerate() {
        ...
        assert!(
            ...
            "Value v{} should have defining op",
            i
        );
    }
...
fn test_large_scale_ir_construction() {
    ...
    let arith_ops: Vec<_> = (0..NUM_OPERATIONS)
        .map(|i| {
    ...
    let const_count = region
        .operations
        .values()
        .filter(|op| op.info.name == "constant")
        .count();
    let add_count = region
        .operations
        .values()
        .filter(|op| op.info.name == "addi")
        .count();
    let mul_count = region
        .operations
        .values()
