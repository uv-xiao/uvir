...
fn test_chained_additions() {
    ...
    // Create constants
    let ops = vec![
        ConstantOp { result: a, value: Attribute::Integer(1) }.into_op_data(&mut ctx),
        ConstantOp { result: b, value: Attribute::Integer(2) }.into_op_data(&mut ctx),
        ConstantOp { result: c, value: Attribute::Integer(3) }.into_op_data(&mut ctx),
        ConstantOp { result: d, value: Attribute::Integer(4) }.into_op_data(&mut ctx),
        AddOp { result: sum1, lhs: a, rhs: b }.into_op_data(&mut ctx), // 1 + 2 = 3
        AddOp { result: sum2, lhs: sum1, rhs: c }.into_op_data(&mut ctx), // 3 + 3 = 6
        AddOp { result: final_sum, lhs: sum2, rhs: d }.into_op_data(&mut ctx), // 6 + 4 = 10
    ];
